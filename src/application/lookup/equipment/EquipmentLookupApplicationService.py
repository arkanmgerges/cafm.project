...
class EquipmentLookupApplicationService(BaseApplicationService):
    def __init__(self, repo: EquipmentLookupRepository):
        self._repo = repo

    @readOnly
    @debugLogger
    def createEquipment(self, *args, **kwargs):
    ...
    @readOnly
    @debugLogger
    def updateEquipment(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
    ...
    @readOnly
    @debugLogger
    def bulkCreateEquipment(self, objListParams: List[dict], token: str = ""):
        objList = []
        exceptions = []
