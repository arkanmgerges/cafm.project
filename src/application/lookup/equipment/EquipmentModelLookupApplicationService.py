...
from src.application.lookup.equipment.EquipmentModelLookupRepository import (
    EquipmentModelLookupRepository,
)
...
class EquipmentModelLookupApplicationService(BaseApplicationService):
    def __init__(self, repo: EquipmentModelLookupRepository):
        self._repo = repo

    @debugLogger
    def createEquipmentModel(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
        obj: EquipmentModel = self._constructObject(*args, **kwargs)
        self._repo.save(obj=obj)

    @debugLogger
    def updateEquipmentModel(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
