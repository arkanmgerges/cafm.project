...
from src.application.lookup.equipment.MaintenanceProcedureOperationLabelRepository import (
    MaintenanceProcedureOperationLabelRepository,
)
...
class MaintenanceProcedureOperationLabelApplicationService(BaseApplicationService):
    def __init__(self, repo: MaintenanceProcedureOperationLabelRepository):
        self._repo = repo

    @readOnly
    @debugLogger
    def createMaintenanceProcedureOperationLabel(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
        obj: MaintenanceProcedureOperationLabel = self._constructObject(*args, **kwargs)
        self._repo.save(obj=obj)

    @readOnly
    @debugLogger
    def updateMaintenanceProcedureOperationLabel(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
    ...
    @readOnly
    @debugLogger
    def deleteMaintenanceProcedureOperationLabel(self, *args, **kwargs):
        _tokenData = TokenService.tokenDataFromToken(token=kwargs["token"])
        kwargs["skipValidation"] = True
        obj: MaintenanceProcedureOperationLabel = self._constructObject(*args, **kwargs)
    ...
    @readOnly
    @debugLogger
    def bulkCreateMaintenanceProcedureOperationLabel(self, objListParams: List[dict], token: str = ""):
        objList = []
        exceptions = []
