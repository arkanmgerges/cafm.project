...
class Manufacturer(HasToMap):
    ...
    def publishDelete(self):
        from src.domain_model.manufacturer.ManufacturerDeleted import (
            ManufacturerDeleted,
        )
    ...
    def publishUpdate(self, old):
        from src.domain_model.manufacturer.ManufacturerUpdated import (
            ManufacturerUpdated,
        )
    ...
    def toMap(self) -> dict:
        return {"manufacturer_id": self.id(), "name": self.name()}
    ...
    def __eq__(self, other):
        if not isinstance(other, Manufacturer):
            raise NotImplementedError(
                f"other: {other} can not be compared with Manufacturer class"
            )
