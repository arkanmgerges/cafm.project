...
class BuildingLevel(HasToMap):
    ...
    def createFrom(
        ...
        **_kwargs,
        ...
        if publishEvent:
            from src.domain_model.project.building.level.BuildingLevelCreated import (
                BuildingLevelCreated,
            )
    ...
    def createFromObject(
        ...
        if obj is None or not isinstance(obj, BuildingLevel):
            raise InvalidArgumentException(
                f"Invalid building level passed as an argument: {obj}"
            )
        id = None if generateNewId else obj.id()
        return cls.createFrom(
            id=id,
            name=obj.name(),
            buildingIds=obj.buildingIds(),
    ...
    def linkBuildingById(self, buildingId: str):
        if buildingId in self._buildingIds:
            ...
            raise BuildingLevelAlreadyLinkedToBuildingException(
                f"Building id: {buildingId}, level: {self}"
            )
        self._buildingIds.append(buildingId)
        from src.domain_model.project.building.level.BuildingLevelToBuildingLinked import (
            BuildingLevelToBuildingLinked,
        )

        DomainPublishedEvents.addEventForPublishing(
            BuildingLevelToBuildingLinked(obj=self, buildingId=buildingId)
        )
    ...
    def unlinkBuildingById(self, buildingId: str):
        ...
        DomainPublishedEvents.addEventForPublishing(
            BuildingLevelToBuildingUnlinked(obj=self, buildingId=buildingId)
        )
    ...
    def addRoom(self, room: BuildingLevelRoom):
        ...
        DomainPublishedEvents.addEventForPublishing(
            BuildingLevelRoomToBuildingLevelAdded(
                buildingLevelRoom=room, buildingLevel=self
            )
        )
    ...
    def publishDelete(self):
        from src.domain_model.project.building.level.BuildingLevelDeleted import (
            BuildingLevelDeleted,
        )
    ...
    def updateRoomIndex(self, roomId: str, index: int):
        ...
        if (
            ...
            DomainPublishedEvents.addEventForPublishing(
                BuildingLevelRoomIndexUpdated(room)
            )
        else:
            raise InvalidArgumentException(
                message=f"Room index is invalid room id: {roomId}, provided index: {index}"
            )
    ...
    def publishUpdate(self, old):
        from src.domain_model.project.building.level.BuildingLevelUpdated import (
            BuildingLevelUpdated,
        )
    ...
    def __eq__(self, other):
        ...
        return (
            self.id() == other.id()
            and self.buildingIds() == other.buildingIds()
