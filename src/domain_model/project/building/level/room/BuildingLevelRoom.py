...
class BuildingLevelRoom(HasToMap):
    ...
    def createFrom(
        ...
        **_kwargs,
        ...
        if publishEvent:
            from src.domain_model.project.building.level.room.BuildingLevelRoomCreated import (
                BuildingLevelRoomCreated,
            )
    ...
    def publishDelete(self):
        from src.domain_model.project.building.level.room.BuildingLevelRoomDeleted import (
            BuildingLevelRoomDeleted,
        )
    ...
    def publishUpdate(self, old):
        from src.domain_model.project.building.level.room.BuildingLevelRoomUpdated import (
            BuildingLevelRoomUpdated,
        )
    ...
    def updateDescription(self, description: str = ""):
        if self.description() != description:
            ...
            DomainPublishedEvents.addEventForPublishing(
                BuildingLevelRoomDescriptionUpdated(self)
            )

    def toMap(self) -> dict:
        return {
            "building_level_room_id": self.id(),
            "name": self.name(),
            "index": self.index(),
