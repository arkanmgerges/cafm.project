...
class DailyCheckProcedureRepository(ABC):
    ...
    @abstractmethod
    def dailyCheckProceduresByEquipmentId(self, tokenData: TokenData, 
                 equipmentId: str = None, 
                 resultFrom: int = 0, 
                 resultSize: int = 100,
                 order: List[dict] = None) -> dict:
        """Get list of daily check procedures by daily_check_procedure id based on the owned roles that the user has

        Args:
            tokenData (TokenData): A token data object
            EquipmentId: A daily_check_procedure id
            resultFrom (int): The start offset of the result item
            resultSize (int): The size of the items in the result
            order (List[dict]): A list of order e.g. [{'orderBy': 'name', 'direction': 'asc'},
                                {'orderBy': 'quantity', 'direction': 'desc'}]
