...
class DailyCheckProcedureRepository(ABC):
    ...
    def bulkSave(self, objList: List[DailyCheckProcedure], tokenData: TokenData = None):
        """Bulk save daily check procedure list

        Args:
            objList (List[DailyCheckProcedure]): The daily check procedure list that needs to be saved
            tokenData (TokenData): Token data that has info about the token

        """

    @abstractmethod
    def bulkDelete(self, objList: List[DailyCheckProcedure], tokenData: TokenData = None):
        """Bulk delete daily check procedure list

        Args:
            objList (List[DailyCheckProcedure]): The daily check procedure list that needs to be deleted
            tokenData (TokenData): Token data that has info about the token
    ...
    def deleteDailyCheckProcedure(
        self, obj: DailyCheckProcedure, tokenData: TokenData
    ) -> None:
    ...
    def dailyCheckProceduresByProjectId(
        ...
        """Get list of daily check procedures by project id based on the owned roles that the user has
