...
class DailyCheckProcedureOperation(HasToMap):
    def __init__(
        self,
        id: str = None,
        name: str = None,
        ...
        **_kwargs,
        ...
        if publishEvent:
            ...
            DomainPublishedEvents.addEventForPublishing(
                DailyCheckProcedureOperationCreated(obj)
            )
        return obj
    ...
    def publishDelete(self):
        from src.domain_model.project.daily_check.procedure.operation.DailyCheckProcedureOperationDeleted import (
            DailyCheckProcedureOperationDeleted,
        )

        DomainPublishedEvents.addEventForPublishing(
            DailyCheckProcedureOperationDeleted(self)
        )

    def publishUpdate(self, old):
        from src.domain_model.project.daily_check.procedure.operation.DailyCheckProcedureOperationUpdated import (
            DailyCheckProcedureOperationUpdated,
        )

        DomainPublishedEvents.addEventForPublishing(
            DailyCheckProcedureOperationUpdated(old, self)
        )

    def toMap(self) -> dict:
        return {
            "daily_check_procedure_operation_id": self.id(),
            "name": self.name(),
            "description": self.description(),
            "type": self.type(),
            "daily_check_procedure_id": self.dailyCheckProcedureId(),
        }
    ...
    def __eq__(self, other):
        if not isinstance(other, DailyCheckProcedureOperation):
            raise NotImplementedError(
                f"other: {other} can not be compared with DailyCheckProcedureOperation class"
            )
        return (
            self.id() == other.id()
            ...
            and self.dailyCheckProcedureId() == other.dailyCheckProcedureId()
        )
