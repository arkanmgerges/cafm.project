...
class EquipmentCategory:
    def __init__(self, id: str = None, name: str = None, skipValidation: bool = False):
        ...
        if not skipValidation:
            if name is None or name == '':
                from src.domain_equipmentCategory.resource.exception.InvalidArgumentException import \
    ...
    def createFrom(cls, id: str = None, name: str = '', publishEvent: bool = False, skipValidation: bool = False):
        from src.domain_model.project.equipment.category.EquipmentCategoryCreated import EquipmentCategoryCreated
        obj = EquipmentCategory(id=id, name=name, skipValidation=skipValidation)

        if publishEvent:
            from src.domain_equipmentCategory.event.DomainPublishedEvents import DomainPublishedEvents
            logger.debug(
    ...
    @classmethod
    def createFromObject(cls, obj: 'EquipmentCategory', publishEvent: bool = False, generateNewId: bool = False,
    ...
    def publishDelete(self):
        from src.domain_model.project.equipment.category.EquipmentCategoryDeleted import EquipmentCategoryDeleted
        DomainPublishedEvents.addEventForPublishing(EquipmentCategoryDeleted(self))

    def publishUpdate(self, old):
        from src.domain_model.project.equipment.category.EquipmentCategoryUpdated import EquipmentCategoryUpdated
    ...
    def __eq__(self, other):
        if not isinstance(other, EquipmentCategory):
            raise NotImplementedError(f'other: {other} can not be compared with EquipmentCategory class')
