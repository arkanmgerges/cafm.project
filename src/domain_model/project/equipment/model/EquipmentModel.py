...
class EquipmentModel(HasToMap):
    ...
    def publishDelete(self):
        from src.domain_model.project.equipment.model.EquipmentModelDeleted import (
            EquipmentModelDeleted,
        )

        DomainPublishedEvents.addEventForPublishing(EquipmentModelDeleted(self))

    def publishUpdate(self, old):
        from src.domain_model.project.equipment.model.EquipmentModelUpdated import (
            EquipmentModelUpdated,
        )
    ...
    def __eq__(self, other):
        if not isinstance(other, EquipmentModel):
            raise NotImplementedError(
                f"other: {other} can not be compared with EquipmentModel class"
            )
