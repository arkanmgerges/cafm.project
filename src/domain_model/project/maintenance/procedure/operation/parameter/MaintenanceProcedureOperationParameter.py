...
class MaintenanceProcedureOperationParameter(HasToMap):
    def __init__(
        self,
        id: str = None,
        name: str = None,
        ...
        if not skipValidation:
            ...
            if (
                maintenanceProcedureOperationId is None
                or maintenanceProcedureOperationId == ""
            ):
                from src.domain_model.resource.exception.InvalidArgumentException import (
                    InvalidArgumentException,
                )
    ...
    def createFrom(
        ...
        if publishEvent:
            ...
            DomainPublishedEvents.addEventForPublishing(
                MaintenanceProcedureOperationParameterCreated(obj)
            )
        return obj
    ...
    def createFromObject(
        ...
        logger.debug(
            f"[{MaintenanceProcedureOperationParameter.createFromObject.__qualname__}]"
        )
    ...
    def publishDelete(self):
        from src.domain_model.project.maintenance.procedure.operation.parameter.MaintenanceProcedureOperationParameterDeleted import (
            MaintenanceProcedureOperationParameterDeleted,
        )

        DomainPublishedEvents.addEventForPublishing(
            MaintenanceProcedureOperationParameterDeleted(self)
        )

    def publishUpdate(self, old):
        from src.domain_model.project.maintenance.procedure.operation.parameter.MaintenanceProcedureOperationParameterUpdated import (
            MaintenanceProcedureOperationParameterUpdated,
        )

        DomainPublishedEvents.addEventForPublishing(
            MaintenanceProcedureOperationParameterUpdated(old, self)
        )

    def toMap(self) -> dict:
        return {
            "maintenance_procedure_operation_parameter_id": self.id(),
            "name": self.name(),
            "unit_id": self.unitId(),
            "maintenance_procedure_operation_id": self.maintenanceProcedureOperationId(),
    ...
    def __eq__(self, other):
        if not isinstance(other, MaintenanceProcedureOperationParameter):
            raise NotImplementedError(
                f"other: {other} can not be compared with MaintenanceProcedureOperationParameter class"
            )
        return (
            self.id() == other.id()
            and self.name() == other.name()
            and self.unitId() == other.unitId()
            and self.maintenanceProcedureOperationId()
            == other.maintenanceProcedureOperationId()
