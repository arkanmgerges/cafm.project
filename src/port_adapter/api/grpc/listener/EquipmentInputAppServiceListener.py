...
resultFrom: {request.resultFrom}, resultSize: {resultSize}, token: {token}"
    ...
    def equipmentInputById(self, request, context):
        try:
            ...
            obj: EquipmentInput = appService.equipmentInputById(
                id=request.id, token=token
            )
            logger.debug(
                f"[{EquipmentInputAppServiceListener.equipmentInputById.__qualname__}] - response: {obj}"
            )
