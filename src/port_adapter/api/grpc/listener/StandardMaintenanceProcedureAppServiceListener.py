...
class StandardMaintenanceProcedureAppServiceListener(StandardMaintenanceProcedureAppServiceServicer, BaseListener):
    ...
    def newId(self, request, context):
        try:
            ...
            logger.debug(
                f"[{StandardMaintenanceProcedureAppServiceListener.newId.__qualname__}] - claims: {claims}\n\t \
                    token: {token}"
            )
    ...
    def standardMaintenanceProcedures(self, request, context):
        try:
            ...
            claims = self._tokenService.claimsFromToken(token=token) if "token" != "" else None
            logger.debug(
                f"[{StandardMaintenanceProcedureAppServiceListener.standardMaintenanceProcedures.__qualname__}] - claims: {claims}\n\t \
resultFrom: {request.resultFrom}, resultSize: {resultSize}, token: {token}"
            ...
            for item in result["items"]:
                response.standardMaintenanceProcedures.add(
                    ...
                    organizationId=item.organizationId() if item.organizationId is not None else "",
                    standardEquipmentCategoryGroupId=item.standardEquipmentCategoryGroupId() if item.standardEquipmentCategoryGroupId is not None else "",
                )
    ...
    def _addObjectToResponse(self, obj: StandardMaintenanceProcedure, response: Any):
        ...
        response.standardMaintenanceProcedure.organizationId = (
            obj.organizationId() if obj.organizationId() is not None else ""
        )
        response.standardMaintenanceProcedure.standardEquipmentCategoryGroupId = (
            obj.standardEquipmentCategoryGroupId() if obj.standardEquipmentCategoryGroupId() is not None else ""
        )
