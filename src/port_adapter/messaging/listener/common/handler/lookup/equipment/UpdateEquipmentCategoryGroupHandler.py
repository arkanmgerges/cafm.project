...
class UpdateEquipmentCategoryGroupHandler(Handler):
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f'[{UpdateEquipmentCategoryGroupHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        equipmentCategoryGroupAppService.updateEquipmentCategoryGroup(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict['token'])
        return None
