...
class DeleteMaintenanceProcedureOperationHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.DELETE_MAINTENANCE_PROCEDURE_OPERATION
    ...
    def handleCommand(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{DeleteMaintenanceProcedureOperationHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.deleteMaintenanceProcedureOperation(
