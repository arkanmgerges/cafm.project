...
class DeleteMaintenanceProcedureOperationLabelHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.DELETE_MAINTENANCE_PROCEDURE_OPERATION_LABEL
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{DeleteMaintenanceProcedureOperationLabelHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.deleteMaintenanceProcedureOperationLabel(
