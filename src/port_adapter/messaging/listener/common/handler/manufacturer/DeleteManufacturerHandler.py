...
class DeleteManufacturerHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.DELETE_MANUFACTURER
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{DeleteManufacturerHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        appService.deleteManufacturer(id=dataDict['id'], token=metadataDict['token'])
