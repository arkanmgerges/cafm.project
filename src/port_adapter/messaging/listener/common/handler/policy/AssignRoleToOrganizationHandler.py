...
class AssignRoleToOrganizationHandler(Handler):
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{AssignRoleToOrganizationHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        appService.assignRoleToOrganization(roleId=dataDict['role_id'], organizationId=dataDict['organization_id'],
                                            token=metadataDict['token'])
        return {'name': self._commandConstant.value, 'created_on': DateTimeHelper.utcNow(),
                'data': {'organization_id': dataDict['organization_id'], 'role_id': dataDict['role_id']},
