...
class RevokeRoleToUserAssignmentHandler(Handler):
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{RevokeRoleToUserAssignmentHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        appService.revokeRoleToUserAssignment(userId=dataDict['user_id'], roleId=dataDict['role_id'],
                                              token=metadataDict['token'])
        return {'name': self._commandConstant.value, 'created_on': DateTimeHelper.utcNow(),
                'data': {'user_id': dataDict['user_id'], 'role_id': dataDict['role_id']},
