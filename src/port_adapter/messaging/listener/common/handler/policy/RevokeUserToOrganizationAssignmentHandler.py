...
class RevokeUserToOrganizationAssignmentHandler(Handler):
    def __init__(self):
        self._commandConstant = (
            CommonCommandConstant.REVOKE_USER_TO_ORGANIZATION_ASSIGNMENT
        )
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{RevokeUserToOrganizationAssignmentHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.revokeUserToOrganizationAssignment(
