...
class UnlinkBuildingLevelFromBuildingHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.UNLINK_BUILDING_LEVEL_FROM_BUILDING
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{UnlinkBuildingLevelFromBuildingHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        appService.unlinkBuildingLevelFromBuilding(buildingLevelId=id, buildingId=dataDict['building_id'],
