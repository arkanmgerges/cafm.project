...
class UnlinkBuildingLevelFromBuildingHandler(Handler):
    def __init__(self):
        self._commandConstant = (
            CommonCommandConstant.UNLINK_BUILDING_LEVEL_FROM_BUILDING
        )
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{UnlinkBuildingLevelFromBuildingHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        data = copy(dataDict)
        appService.unlinkBuildingLevelFromBuilding(
