...
class CreateBuildingLevelRoomHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.CREATE_BUILDING_LEVEL_ROOM
    ...
    def handleCommand(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{CreateBuildingLevelRoomHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        data = copy(dataDict)
        dataDict["id"] = dataDict.pop("building_level_room_id")
        appService.createBuildingLevelRoom(
