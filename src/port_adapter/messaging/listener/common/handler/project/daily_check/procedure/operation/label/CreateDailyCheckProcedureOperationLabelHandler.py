...
class CreateDailyCheckProcedureOperationLabelHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.CREATE_DAILY_CHECK_PROCEDURE_OPERATION_LABEL
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f'[{CreateDailyCheckProcedureOperationLabelHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        obj: DailyCheckProcedureOperationLabel = dailyCheckProcedureOperationLabelAppService.createDailyCheckProcedureOperationLabel(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict['token'])
