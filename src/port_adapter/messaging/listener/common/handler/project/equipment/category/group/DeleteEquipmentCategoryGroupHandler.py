...
class DeleteEquipmentCategoryGroupHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.DELETE_EQUIPMENT_CATEGORY_GROUP
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{DeleteEquipmentCategoryGroupHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        return {'name': self._commandConstant.value, 'created_on': DateTimeHelper.utcNow(),
                'data': {'id': dataDict['equipment_category_group_id']},
