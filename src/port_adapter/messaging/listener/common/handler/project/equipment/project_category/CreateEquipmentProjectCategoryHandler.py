...
class CreateEquipmentProjectCategoryHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.CREATE_EQUIPMENT_PROJECT_CATEGORY
    ...
    def handleCommand(self, messageData: dict) -> dict:
        ...
        logger.debug(
            f'[{CreateEquipmentProjectCategoryHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
