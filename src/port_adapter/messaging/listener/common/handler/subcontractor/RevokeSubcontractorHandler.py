...
class RevokeSubcontractorHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.REVOKE_ASSIGNMENT_SUBCONTRACTOR_TO_ORGANIZATION
    ...
    def handleCommand(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{RevokeSubcontractorHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.revokeSubcontractor(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict["token"])
