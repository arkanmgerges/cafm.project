...
class ProjectCommandListener:
    ...
    def run(self):
        ...
        try:
                ...
                if msg.error():
                    ...
                    while not isMsgProcessed:
                        ...
                        try:
                            msgData = msg.value()
                            logger.debug(
                                f"[{ProjectCommandListener.run.__qualname__}] received message data = {msgData}"
                            )
                            ...
                            logger.debug(
                                f"[{ProjectCommandListener.run.__qualname__}] cleanup event publisher"
                            )
            ...
            logger.info(
                f"[{ProjectCommandListener.run.__qualname__}] Shutting down the process"
            )
        finally:
