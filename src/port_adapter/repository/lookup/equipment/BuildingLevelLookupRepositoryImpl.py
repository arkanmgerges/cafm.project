...
from src.application.lookup.equipment.BuildingLevelLookupRepository import BuildingLevelLookupRepository
...
class BuildingLevelLookupRepositoryImpl(BuildingLevelLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{BuildingLevelLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def save(self, obj: BuildingLevel):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="ctx._source.building_level.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
