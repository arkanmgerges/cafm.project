...
from src.application.lookup.equipment.BuildingLevelRoomLookupRepository import BuildingLevelRoomLookupRepository
...
class BuildingLevelRoomLookupRepositoryImpl(BuildingLevelRoomLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{BuildingLevelRoomLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def save(self, obj: BuildingLevelRoom):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="ctx._source.building_level_room.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
