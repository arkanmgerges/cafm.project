...
from src.application.lookup.equipment.BuildingLookupRepository import BuildingLookupRepository
...
class BuildingLookupRepositoryImpl(BuildingLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{BuildingLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
        ...
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             .filter('nested', path="building",
                     query=Q("term",
                             **{"building.id": obj.id()})) \
             .script(source="ctx._source.building.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
