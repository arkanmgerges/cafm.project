...
from src.application.lookup.equipment.EquipmentCategoryGroupLookupRepository import EquipmentCategoryGroupLookupRepository
...
class EquipmentCategoryGroupLookupRepositoryImpl(EquipmentCategoryGroupLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{EquipmentCategoryGroupLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def save(self, obj: EquipmentCategoryGroup):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="ctx._source.equipment_category_group.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
