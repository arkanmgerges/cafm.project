...
class EquipmentCategoryGroupRepositoryImpl(EquipmentCategoryGroupRepository):
    ...
    def delete(self, obj: EquipmentCategoryGroup):
        if obj is not None:
            Search(index=EsEquipment.alias()).using(self._es).filter('nested', path="equipment_category_group", query=Q("term", **{"equipment_category_group.id": obj.id()})).delete()
    ...
    def save(self, obj: EquipmentCategoryGroup):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="""
             if (params.name != null) {
                            ctx._source.equipment_category_group.name = params.name;
                        }
             """, params={
                 "name": obj.name(),
                 }) \
