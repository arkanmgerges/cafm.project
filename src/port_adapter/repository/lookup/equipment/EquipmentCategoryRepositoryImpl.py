...
from src.application.lookup.equipment.EquipmentCategoryRepository import EquipmentCategoryRepository
...
class EquipmentCategoryRepositoryImpl(EquipmentCategoryRepository):
    ...
    def delete(self, obj: EquipmentCategory):
        if obj is not None:
            Search(index=EsEquipment.alias()).using(self._es).filter('nested', path="equipment_category", query=Q("term", **{"equipment_category.id": obj.id()})).delete()
    ...
    def save(self, obj: EquipmentCategory):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="""
             if (params.name != null) {
                            ctx._source.equipment_category.name = params.name;
                        }
             """, params={
                 "name": obj.name(),
                 }) \
