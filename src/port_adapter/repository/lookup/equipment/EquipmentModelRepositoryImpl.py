...
class EquipmentModelRepositoryImpl(EquipmentModelRepository):
    ...
    def delete(self, obj: EquipmentModel):
        if obj is not None:
            Search(index=EsEquipment.alias()).using(self._es).filter('nested', path="equipment_model", query=Q("term", **{"equipment_model.id": obj.id()})).delete()
    ...
    def save(self, obj: EquipmentModel):
        if obj is not None:
            UpdateByQueryValidator.validate(UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="""
             if (params.name != null) {
                            ctx._source.equipment_model.name = params.name;
                        }
             """, params={
                 "name": obj.name(),
                 }) \
