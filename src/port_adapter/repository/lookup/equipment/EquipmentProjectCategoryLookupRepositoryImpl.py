...
from src.application.lookup.equipment.EquipmentProjectCategoryLookupRepository import EquipmentProjectCategoryLookupRepository
...
class EquipmentProjectCategoryLookupRepositoryImpl(EquipmentProjectCategoryLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{EquipmentProjectCategoryLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def save(self, obj: EquipmentProjectCategory):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="ctx._source.equipment_project_category.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
