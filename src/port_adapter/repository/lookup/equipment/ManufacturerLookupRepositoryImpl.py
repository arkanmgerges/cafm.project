...
from src.application.lookup.equipment.ManufacturerLookupRepository import ManufacturerLookupRepository
...
class ManufacturerLookupRepositoryImpl(ManufacturerLookupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{ManufacturerLookupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def save(self, obj: Manufacturer):
        if obj is not None:
            UpdateByQuery(index=EsEquipment.alias()).using(self._es) \
             ...
             .script(source="ctx._source.manufacturer.name = params.name;", params={
                 "name": obj.name(),
                 }) \
            .execute()
