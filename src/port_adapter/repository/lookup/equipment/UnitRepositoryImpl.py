...
from src.application.lookup.equipment.UnitRepository import UnitRepository
...
class UnitRepositoryImpl(UnitRepository):
    ...
    def delete(self, obj: Unit):
        if obj is not None:
            Search(index=EsEquipment.alias()).using(self._es).filter('nested', path="unit", query=Q("term", **{"unit.id": obj.id()})).delete()
