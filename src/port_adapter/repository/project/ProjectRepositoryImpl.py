...
class ProjectRepositoryImpl(ProjectRepository):
        ...
        result = dbSession.execute(
            text(
                f"""SELECT project.name project_name, project.id project_id, project.created_at project_created_at, 
                        project.modified_at project_modified_at, 
    ...
    @debugLogger
    def _projectStatisticFromDbObject(self, dbObject):
        return ProjectStatistic(
            projectId=dbObject.project_id,
            projectName=dbObject.project_name,
            projectCreatedAt=DateTimeHelper.datetimeToInt(dbObject.project_created_at) if dbObject.project_created_at is not None else 0,
            projectModifiedAt=DateTimeHelper.datetimeToInt(dbObject.project_modified_at) if dbObject.project_modified_at is not None else 0,
            buildingCount=dbObject.building_count,
            equipmentCount=dbObject.equipment_count,
            maintenanceProcedureCount=dbObject.maintenance_procedure_count,
        )