...
class ProjectRepositoryImpl(ProjectRepository):
    ...
    def createProject(self, obj: Project, tokenData: TokenData = None):
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = DbProject(id=obj.id(), name=obj.name(), cityId=obj.cityId(), countryId=obj.countryId(), startDate=obj.startDate(), beneficiaryId=obj.beneficiaryId(), addressLine=obj.addressLine(), state=obj.state().value)
    ...
    @debugLogger
    def projects(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None, tokenData: TokenData = None) -> dict:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            sortData = ''
    ...
    def changeState(self, project: Project, tokenData: TokenData) -> None:
        ...
        try:
            ...
            if project.state() is ProjectState.ACTIVE:
                dbObject.startDate = project.startDate()
            dbSession.add(dbObject)
            dbSession.commit()
        finally:
            dbSession.close()
