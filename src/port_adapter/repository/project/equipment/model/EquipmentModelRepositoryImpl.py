...
class EquipmentModelRepositoryImpl(EquipmentModelRepository):
    def __init__(self):
        try:
            ...
            logger.warn(f'[{EquipmentModelRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}')
    ...
    def updateEquipmentModel(self, obj: EquipmentModel, tokenData: TokenData = None) -> None:
        ...
        try:
            ...
            if savedObj == obj:
                logger.debug(
                    f'[{EquipmentModelRepositoryImpl.updateEquipmentModel.__qualname__}] Object identical exception for old equipment model: {savedObj}\nequipment model: {obj}')
    ...
    def equipmentModels(self, tokenData: TokenData, resultFrom: int = 0, resultSize: int = 100,
        ...
        try:
            ...
            if order is not None:
                ...
                sortData = sortData[2:]
            items = dbSession.query(DbEquipmentModel).order_by(text(sortData)).limit(resultSize).offset(resultFrom).all()
