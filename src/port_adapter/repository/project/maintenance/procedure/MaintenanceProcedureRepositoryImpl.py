...
class MaintenanceProcedureRepositoryImpl(MaintenanceProcedureRepository):
    ...
    def maintenanceProcedureById(self, id: str) -> MaintenanceProcedure:
        ...
        try:
            ...
            return MaintenanceProcedure.createFrom(id=dbObject.id, name=dbObject.name, type=dbObject.type, frequency=dbObject.frequency, startDate=DateTimeHelper.datetimeToInt(dbObject.startDate), subcontractorId=dbObject.subcontractorId, equipmentId=dbObject.equipmentId)
        finally:
            dbSession.close()
    ...
    def maintenanceProcedures(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None, tokenData: TokenData = None) -> dict:
        ...
        try:
            ...
            return {"items": [MaintenanceProcedure.createFrom(id=x.id, name=x.name, type=x.type, frequency=x.frequency, startDate=DateTimeHelper.datetimeToInt(x.startDate), subcontractorId=x.subcontractorId, equipmentId=x.equipmentId) for x in items],
