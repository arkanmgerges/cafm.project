...
class MaintenanceProcedureOperationRepositoryImpl(MaintenanceProcedureOperationRepository):
    ...
    def createMaintenanceProcedureOperation(self, obj: MaintenanceProcedureOperation, tokenData: TokenData = None):
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = DbMaintenanceProcedureOperation(id=obj.id(), name=obj.name(), description=obj.description(),
                                                       type=obj.type(),
    ...
    @debugLogger
    def deleteMaintenanceProcedureOperation(self, obj: MaintenanceProcedureOperation,
    ...
    @debugLogger
    def updateMaintenanceProcedureOperation(self, obj: MaintenanceProcedureOperation,
                                            tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbMaintenanceProcedureOperation).filter_by(id=obj.id()).first()
    ...
    def maintenanceProcedureOperationById(self, id: str) -> MaintenanceProcedureOperation:
        ...
        try:
            ...
            return MaintenanceProcedureOperation.createFrom(id=dbObject.id, name=dbObject.name,
                                                            description=dbObject.description, type=dbObject.type,
    ...
    def maintenanceProcedureOperations(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None,
        ...
        try:
            ...
            if order is not None:
                ...
                sortData = sortData[2:]
            items = dbSession.query(DbMaintenanceProcedureOperation).order_by(text(sortData)).limit(resultSize).offset(
                resultFrom).all()
            itemsCount = dbSession.query(DbMaintenanceProcedureOperation).count()
            if items is None:
                return {"items": [], "itemCount": 0}
            return {"items": [
                MaintenanceProcedureOperation.createFrom(id=x.id, name=x.name, description=x.description, type=x.type,
                                                         maintenanceProcedureId=x.maintenanceProcedureId) for x in
                items],
    ...
    @debugLogger
    def maintenanceProcedureOperationsByMaintenanceProcedureId(self, maintenanceProcedureId: str = None,
        ...
        try:
            ...
            if order is not None:
                for item in order:
                    sortData = f'{sortData}, {item["orderBy"]} {item["direction"]}'
                sortData = sortData[2:]
            items = dbSession.query(DbMaintenanceProcedureOperation).filter_by(
                maintenanceProcedureId=maintenanceProcedureId).order_by(text(sortData)).limit(resultSize).offset(
                resultFrom).all()
            itemsCount = dbSession.query(DbMaintenanceProcedureOperation).filter_by(
                maintenanceProcedureId=maintenanceProcedureId).count()
            if items is None:
                return {"items": [], "itemCount": 0}
            return {"items": [
                MaintenanceProcedureOperation.createFrom(id=x.id, name=x.name, description=x.description, type=x.type,
