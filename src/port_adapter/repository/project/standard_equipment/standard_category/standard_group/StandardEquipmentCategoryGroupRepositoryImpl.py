...
class StandardEquipmentCategoryGroupRepositoryImpl(StandardEquipmentCategoryGroupRepository):
    def __init__(self):
        try:
    ...
    def save(self, obj: StandardEquipmentCategoryGroup, tokenData: TokenData = None):
        ...
        try:
            ...
            if dbObject is not None:
                self.updateStandardEquipmentCategoryGroup(obj=obj, tokenData=tokenData)
            else:
    ...
    @debugLogger
    def createStandardEquipmentCategoryGroup(self, obj: StandardEquipmentCategoryGroup, tokenData: TokenData = None):
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = DbStandardEquipmentCategoryGroup(id=obj.id(),
                                                        name=obj.name(),
                                                        standardEquipmentCategoryId=obj.standardEquipmentCategoryId())
            result = dbSession.query(DbStandardEquipmentCategoryGroup).filter_by(id=obj.id()).first()
            if result is None:
    ...
    @debugLogger
    def deleteStandardEquipmentCategoryGroup(self, obj: StandardEquipmentCategoryGroup,
    ...
    @debugLogger
    def updateStandardEquipmentCategoryGroup(self, obj: StandardEquipmentCategoryGroup,
                                             tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardEquipmentCategoryGroup).filter_by(id=obj.id()).first()
            if dbObject is None:
                raise StandardEquipmentCategoryGroupDoesNotExistException(f'id = {obj.id()}')
            savedObj: StandardEquipmentCategoryGroup = self.standardEquipmentCategoryGroupById(obj.id())
            if savedObj != obj:
    ...
    @debugLogger
    def standardEquipmentCategoryGroupById(self, id: str) -> StandardEquipmentCategoryGroup:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardEquipmentCategoryGroup).filter_by(id=id).first()
            if dbObject is None:
                raise StandardEquipmentCategoryGroupDoesNotExistException(f'id = {id}')
            return StandardEquipmentCategoryGroup.createFrom(id=dbObject.id,
                                                             name=dbObject.name,
    ...
    def standardEquipmentCategoryGroups(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None,
        ...
        try:
            ...
            if order is not None:
                ...
                sortData = sortData[2:]
            items = dbSession.query(DbStandardEquipmentCategoryGroup).order_by(text(sortData)).limit(resultSize).offset(
                resultFrom).all()
            itemsCount = dbSession.query(DbStandardEquipmentCategoryGroup).count()
            if items is None:
                return {"items": [], "itemCount": 0}
            return {"items": [StandardEquipmentCategoryGroup.createFrom(id=x.id,
                                                                        name=x.name,
