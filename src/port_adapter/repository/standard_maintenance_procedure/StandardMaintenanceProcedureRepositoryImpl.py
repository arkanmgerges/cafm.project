...
class StandardMaintenanceProcedureRepositoryImpl(StandardMaintenanceProcedureRepository):
    ...
    def createStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure, tokenData: TokenData = None):
        ...
        try:
            dbObject = DbStandardMaintenanceProcedure(id=obj.id(),
                                                      ...
                                                      startDate=DateTimeHelper.intToDateTime(
    ...
    @debugLogger
    def deleteStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure,
    ...
    @debugLogger
    def updateStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure,
                                           tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardMaintenanceProcedure).filter_by(id=obj.id()).first()
    ...
    def standardMaintenanceProcedureById(self, id: str) -> StandardMaintenanceProcedure:
        ...
        try:
            ...
            return StandardMaintenanceProcedure.createFrom(id=dbObject.id,
                                                           ...
                                                           startDate=DateTimeHelper.datetimeToInt(
                                                               dbObject.startDate) if DateTimeHelper.datetimeToInt(
                                                               dbObject.startDate) is not None else 0,
    ...
    def standardMaintenanceProcedures(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None,
        ...
        try:
            ...
            if order is not None:
                ...
                sortData = sortData[2:]
            items = dbSession.query(DbStandardMaintenanceProcedure).order_by(text(sortData)).limit(resultSize).offset(
                resultFrom).all()
            ...
            return {"items": [StandardMaintenanceProcedure.createFrom(id=x.id,
                                                                      ...
                                                                      startDate=DateTimeHelper.datetimeToInt(
                                                                          x.startDate) if DateTimeHelper.datetimeToInt(
                                                                          x.startDate) is not None else 0,
                                                                      organizationId=x.organizationId,
                                                                      standardEquipmentCategoryGroupId=x.standardEquipmentCategoryGroupId) for x in items],
