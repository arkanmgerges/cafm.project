...
from src.port_adapter.repository.db_model.StandardMaintenanceProcedure import StandardMaintenanceProcedure as DbStandardMaintenanceProcedure
from src.resource.common.DateTimeHelper import DateTimeHelper
from src.resource.logging.decorator import debugLogger
from src.resource.logging.logger import logger


class StandardMaintenanceProcedureRepositoryImpl(StandardMaintenanceProcedureRepository):
    def __init__(self):
        try:
            self._db = create_engine(
                f"mysql+mysqlconnector://{os.getenv('CAFM_PROJECT_DB_USER', 'root')}:{os.getenv('CAFM_PROJECT_DB_PASSWORD', '1234')}@{os.getenv('CAFM_PROJECT_DB_HOST', '127.0.0.1')}:{os.getenv('CAFM_PROJECT_DB_PORT', '3306')}/{os.getenv('CAFM_PROJECT_DB_NAME', 'cafm-project')}")
        except Exception as e:
            logger.warn(f'[{StandardMaintenanceProcedureRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}')
    ...
    @debugLogger
    def createStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure, tokenData: TokenData = None):
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = DbStandardMaintenanceProcedure(id=obj.id(), 
			name=obj.name(),
			type=obj.type(),
			subtype=obj.subtype(),
            ...
            result = dbSession.query(DbStandardMaintenanceProcedure).filter_by(id=obj.id()).first()
            if result is None:
    ...
    @debugLogger
    def deleteStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure, tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardMaintenanceProcedure).filter_by(id=obj.id()).first()
            if dbObject is not None:
    ...
    @debugLogger
    def updateStandardMaintenanceProcedure(self, obj: StandardMaintenanceProcedure, tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardMaintenanceProcedure).filter_by(id=obj.id()).first()
            if dbObject is None:
                raise StandardMaintenanceProcedureDoesNotExistException(f'id = {obj.id()}')
            savedObj: StandardMaintenanceProcedure = self.standardMaintenanceProcedureById(obj.id())
            if savedObj != obj:
    ...
    def standardMaintenanceProcedureById(self, id: str) -> StandardMaintenanceProcedure:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbStandardMaintenanceProcedure).filter_by(id=id).first()
            if dbObject is None:
                raise StandardMaintenanceProcedureDoesNotExistException(f'id = {id}')
            return StandardMaintenanceProcedure.createFrom(id=dbObject.id, 
			name=dbObject.name,
			type=dbObject.type,
    ...
    @debugLogger
    def standardMaintenanceProcedures(self, resultFrom: int = 0, resultSize: int = 100, order: List[dict] = None, tokenData: TokenData = None) -> dict:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            sortData = ''
            if order is not None:
                ...
                sortData = sortData[2:]
            items = dbSession.query(DbStandardMaintenanceProcedure).order_by(text(sortData)).limit(resultSize).offset(resultFrom).all()
            itemsCount = dbSession.query(DbStandardMaintenanceProcedure).count()
            if items is None:
                return {"items": [], "itemCount": 0}
            return {"items": [StandardMaintenanceProcedure.createFrom(id=x.id, 
			name=x.name,
			type=x.type,
			subtype=x.subtype,
			frequency=x.frequency,
            startDate=DateTimeHelper.datetimeToInt(x.startDate) if DateTimeHelper.datetimeToInt(x.startDate) is not None else 0,
			organizationId=x.organizationId) for x in items],
