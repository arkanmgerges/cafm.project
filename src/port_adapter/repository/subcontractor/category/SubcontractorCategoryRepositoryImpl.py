...
class SubcontractorCategoryRepositoryImpl(SubcontractorCategoryRepository):
    ...
    def save(self, obj: SubcontractorCategory, tokenData: TokenData = None):
        ...
        try:
            ...
            if dbObject is not None:
                self.updateSubcontractorCategory(obj=obj, tokenData=tokenData)
            else:
    ...
    def createSubcontractorCategory(self, obj: SubcontractorCategory, tokenData: TokenData = None):
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = DbSubcontractorCategory(id=obj.id(), 
			name=obj.name())
            result = dbSession.query(DbSubcontractorCategory).filter_by(id=obj.id()).first()
            if result is None:
    ...
    @debugLogger
    def updateSubcontractorCategory(self, obj: SubcontractorCategory, tokenData: TokenData = None) -> None:
        dbSession = DbSession.newSession(dbEngine=self._db)
        try:
            dbObject = dbSession.query(DbSubcontractorCategory).filter_by(id=obj.id()).first()
            if dbObject is None:
                raise SubcontractorCategoryDoesNotExistException(f'id = {obj.id()}')
            savedObj: SubcontractorCategory = self.subcontractorCategoryById(obj.id())
            if savedObj != obj:
