...
def test_create_object():
    ...
    obj = _create_object()
    # Assert
    assert isinstance(obj, EquipmentCategory)
...
def test_create_from_object():
    # Act
    obj = _create_object()
    obj2 = EquipmentCategory.createFromObject(obj=obj)
    # Assert
    assert obj == obj2
...
def test_invalid_name():
    ...
    with pytest.raises(InvalidArgumentException):
        EquipmentCategory.createFrom(name=None)
    with pytest.raises(InvalidArgumentException):
        EquipmentCategory.createFrom(name='')
