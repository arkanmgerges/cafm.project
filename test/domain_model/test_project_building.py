...
def test_add_level():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[0], BuildingLevelToBuildingAdded
    )
...
def test_remove_level():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[0], BuildingLevelToBuildingRemoved
    )
...
def test_throw_level_already_exist_when_adding_same_level_to_building():
    ...
    with pytest.raises(BuildingLevelAlreadyExistException):
        obj2 = BuildingLevel.createFrom(buildingIds=[buildingId])
        obj = Building.createFrom(
            id=buildingId, projectId=projectId, buildingLevels=[obj2]
        )
        obj.addLevel(level=obj2)
...
def test_link_two_buildings_to_one_level():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[0], BuildingLevelToBuildingLinked
    )
...
def test_unlink_building_from_building_level():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[1], BuildingLevelToBuildingUnlinked
    )
