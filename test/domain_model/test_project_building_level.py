...
def test_add_room_to_level():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[0],
        BuildingLevelRoomToBuildingLevelAdded,
    )
...
def test_throw_exception_when_adding_same_room_to_level():
    ...
    with pytest.raises(BuildingLevelAlreadyHasRoomException):
        ...
        obj = BuildingLevel.createFrom(
            id=buildingLevelId, buildingIds=[buildingId], rooms=[r1]
        )
        obj.addRoom(r1)
...
def test_change_description():
    ...
    assert isinstance(
        DomainPublishedEvents.postponedEvents()[0], BuildingLevelRoomDescriptionUpdated
    )
